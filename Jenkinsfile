    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/Prkhr01/webcalc-java.git'
            }
        }
        ...
        stage('Deploy to Tomcat') {
            steps {
                ...
                deploy adapters: [tomcat8(credentialsId: 'tomcat', path: '', url: 'http://192.168.50.137:8081/')], contextPath: null, war: '**/*.war'
            }
        }
        ...
        //stage('API Testing') {
            //steps {
                //script {
                    ...
                    //def getResponse = sh(script: 'curl -X GET http://192.168.79.130:8081/webapp-0.2/', returnStdout: true).trim()
                    //def postResponse = sh(script: 'curl -X POST -d "n1=5&n2=6&r1=add" http://192.168.79.130:8081/webapp-0.2/firstHomePage', returnStdout: true).trim()
